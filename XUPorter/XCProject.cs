...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		*/
		private bool processTarget(string target, string team)
		{
			PBXDictionary dict_ = project.data;
			if(dict_.ContainsKey("attributes"))
			{
				Debug.Log("[tsq]find attributes 0 ");
				PBXDictionary attributes_ = (PBXDictionary)dict_ ["attributes"];
				if(attributes_.ContainsKey("TargetAttributes"))
				{
					Debug.Log("[tsq]find TargetAttributes 1");
					PBXDictionary targetAttributes_ = (PBXDictionary)attributes_["TargetAttributes"];
					if(targetAttributes_.ContainsKey(target))
					{ 
						Debug.Log("[tsq]find target: " + target + " 2");
						PBXDictionary targetValue = (PBXDictionary)targetAttributes_[target];
					
						if(team != null && team != "")
						{
							if(!targetValue.ContainsKey("DevelopmentTeam"))
							{
								targetValue.Add("DevelopmentTeam", team);
							}
						}

						if(targetValue.ContainsKey("SystemCapabilities"))
						{
							PBXDictionary systemCapabilite_ = (PBXDictionary)targetValue["SystemCapabilities"];
							PBXDictionary enable_ = new PBXDictionary();
							enable_.Add("enabled", "1");
							systemCapabilite_.Add("com.apple.Push", enable_);
						}
						else
						{
							PBXDictionary enable_ = new PBXDictionary();
							enable_.Add("enabled", "1");
							PBXDictionary com_apple_Push = new PBXDictionary();
							com_apple_Push.Add("com.apple.Push", enable_);
							
							targetValue.Add("SystemCapabilities", com_apple_Push);
						}

					}
					else
					{
						Debug.Log("[tsq]new target: " + target + " 2");
						PBXDictionary targetValue = new PBXDictionary();
						...
						targetAttributes_.Add(target, targetValue);
					}
				}
			}
			return true;
		...
		public bool EnablePushNotification(string team = null, bool flag = true)
		{
			PBXDictionary dict_ = project.data;
			if (dict_.ContainsKey ("targets")) {
				Debug.Log("[tsq] find targets");
				PBXList targets_ = (PBXList)dict_["targets"];
				foreach(string target in targets_)
				{
					Debug.Log("[tsq]process target:" + target);
					processTarget(target, team);
				}
			}
			Debug.Log ("[tsq]EnablePushNotification end");
			return true;
		}
